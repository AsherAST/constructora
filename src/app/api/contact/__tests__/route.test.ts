...
describe("POST /api/contact", () => {
  ...
  test("envía el correo y responde ok con datos válidos", async () => {
    ...
    expect(mockSend).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        subject: "Nuevo mensaje de contacto: Ana Torres",
        text: expect.stringContaining("Quiero un presupuesto"),
      }),
    );
  ...
  test("responde 500 si el envío del correo falla", async () => {
    mockSend.mockResolvedValueOnce({ data: null, error: new Error("boom") });
